...
void wifiManagerSetup() {
    ...
    const char* custom_radio_str =
        ...
        "type='radio' name='customfieldid' value='2'> Two<br><input "
        "type='radio' name='customfieldid' value='3'> Three";
    new (&custom_field)
    ...
    // (true) wm.setMinimumSignalQuality(20);  // set min RSSI (percentage) to
    // show in scans, null = 8% wm.setShowInfoErase(false);      // do not show
    // erase button on info page wm.setScanDispPerc(true);       // show RSSI as
    ...
    // res = wm.autoConnect("universalIncubator", "password"); // password
    // protected ap
...
void wifiManagerLoop() {
    if (wm_nonblocking)
        wm.process();  // avoid delays() in loop when non-blocking and other
                       // long running code
    checkButton();
}
...
void checkButton() {
    ...
    if (digitalRead(TRIGGER_PIN) == LOW) {
        ...
        if (digitalRead(TRIGGER_PIN) == LOW) {
            ...
            // still holding button for 3000 ms, reset settings, code not ideaa
            // for production
